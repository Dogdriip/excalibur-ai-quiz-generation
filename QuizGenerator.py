...
class SentenceTokenizer(object):
    ...
    def text2sentences(self, text):
        ...
        for idx in range(len(sentences)):
            # if length of current sentence is lte 10, concat next sentence with current sentence.
    ...
    def get_nouns(self, sentences):
        ...
        for sentence in sentences:
            if sentence != "":
                nouns.append(" ".join([noun for noun in self.okt.nouns(
                    str(sentence)) if noun not in self.stopwords and len(noun) > 1]))
...
class GraphMatrix(object):
    ...
    def build_sent_graph(self, sentence):
        # Sentence-Term Matrix (derived from DTM, Document-Term Matrix)
        tfidf_mat = self.tfidf.fit_transform(sentence).toarray()
        # Correlation Matrix among sentences.
...
class TextRank(object):
    def __init__(self, text):
        ...
        # Build sentence graph (adjacency matrix) with given nouns
        self.sent_graph = self.graph_matrix.build_sent_graph(self.nouns)
        self.words_graph, self.idx2word = self.graph_matrix.build_words_graph(
            self.nouns)  # Build words graph(?) with given nouns
